...
- **Not yet done**: GPU acceleration (taco has ROCm already set up for llama.cpp, but the inference server currently uses `CPUExecutionProvider` — CPU inference is already fast enough for the nano models at ~15-35ms, so this hasn't been a priority) and license plate detection/OCR — see `docs/PLAN-realtime-license-plate-detection.md` for the plan (dataset already gathered in `data/license_plates/`, not yet trained).
