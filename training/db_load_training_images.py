...
submitted_images/detection_labels tables (inference-server/orm.py), so the
label-quality investigation (compute_label_confidence.py) and the DB-driven
training refactor (train_from_db.py) have real rows to work against instead
of flat YOLO .txt files.

Why reuse submitted_images/detection_labels rather than a new table: the
user's own framing was "load into the database of submitted images" -
source tags ("kitti"/"coco128"/"license_plates") distinguish bulk-imported
training data from real endpoint traffic in the same table, the same way
Tag already distinguishes anything else.

Per-source label provenance (see orm.py's DetectionLabel.label_source):
...
- KITTI: labels/{split}/*.txt is KITTI's own 8-class human annotation
  ...
  reused from train_resume_all_categories.py) -> label_source="human_dataset",
  dataset=Dataset("KITTI"). labels_license_plate_only/{split}/*.txt is a
  SEPARATE directory entirely produced by label_plates_via_api.py calling
  this server's own /alpr/predict -> label_source="machine",
  model_name="fast-alpr" (kept apart from KITTI's own labels/ on disk
  specifically because it isn't part of KITTI's ground truth).
- COCO128: labels/train2017/*.txt mixes COCO128's own human-annotated boxes
  ...
  class_id == LICENSE_PLATE_CLASS_ID -> label_source="machine",
  model_name="fast-alpr"; every other class id -> label_source=
  "human_dataset", dataset=Dataset("COCO128").
- data/license_plates/: labels_plates_only/*.txt is the original human-drawn
  plate box(es) (label_app.py) -> label_source="human_dataset",
  dataset=Dataset("license_plates-manual"). labels/*.txt is pseudo_label_
  plates.py's regenerated superset (the same plate box(es) plus yolo12n.pt's
  ...
  else is machine (label_source="machine", model_name="yolo12n.pt").
...
from orm import Base, Dataset, DetectionLabel, SubmittedImage, Tag, engine_from_env  # noqa: E402
from persist import _detection_label, _store_image, _store_thumbnail  # noqa: E402
...
def _get_or_create_dataset(session: Session, name: str, description: str) -> Dataset:
    dataset = session.query(Dataset).filter_by(name=name).one_or_none()
    if dataset is None:
...
def _already_loaded(session: Session, sha256: str, source_tag: Tag) -> bool:
    return (
        ...
        .filter(SubmittedImage.sha256 == sha256, SubmittedImage.tags.contains(source_tag))
        .first()
        is not None
    )
...
def _parse_yolo_label_file(label_path: Path) -> list[tuple[int, tuple[float, float, float, float]]]:
    ...
    persist._detection_label's `det["box"]` expects."""
...
def _add_labels(
    ...
    *,
    ...
    for class_id, box in boxes:
        ...
        label = _detection_label(
            submitted.id,
            det,
            model_name=None if is_human else machine_model_name,
            source="server",
            label_source="human_dataset" if is_human else "machine",
            dataset_id=human_dataset.id if is_human else None,
        )
        session.add(label)
    ...
    for split in ("train", "val"):
        ...
        for image_path in image_paths:
            ...
            _add_labels(
                ...
                human_dataset=dataset, machine_model_name="", remap=KITTI_TO_UNIFIED,
            )
            ...
            for _, box in plate_boxes:
                ...
                session.add(_detection_label(
                    submitted.id, det, model_name="fast-alpr", source="server",
                    label_source="machine", dataset_id=None,
                ))
            session.commit()
            count += 1
    return count
...
def load_coco128(session: Session, limit: int | None) -> int:
    ...
    for image_path in sorted(images_dir.glob("*.jpg")):
        ...
        _add_labels(
            ...
            human_dataset=dataset, machine_model_name="fast-alpr",
        )
        session.commit()
        count += 1
    return count
...
def load_license_plates(session: Session, limit: int | None) -> int:
    ...
    for image_path in sorted(images_dir.glob("*.jpg")) + sorted(images_dir.glob("*.png")):
        ...
        _add_labels(
            session, submitted, boxes, human_class_ids=human_class_ids,
            human_dataset=dataset, machine_model_name="yolo12n.pt",
        )
        session.commit()
        count += 1
    return count
